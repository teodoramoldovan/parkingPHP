<?php  	
		...
		if(isset ($_POST["submit"])){
			if(empty($_POST["parkingName"]) || empty($_POST["parkingAddress"]||
				empty($_POST["numberOfPlaces"] || empty($_POST["freePlaces"]) )))  
			{  
			...
			else
			{
				...
				$freePlaces=mysqli_real_escape_string($connect, $_POST["freePlaces"]);
				$id=mysqli_real_escape_string($connect,$_POST["id"]);
				
				$query2="UPDATE parkings SET name='$parkingName',address='$parkingAddress',
					numberOfPlaces='$numberOfPlaces',freePlaces='$freePlaces' WHERE id='$id'";
...
<br><br><br>
<h2 align="center"><font size="6">Update parking</font></h2>

<form method="POST" action="updateParking" modelAttribute="updatedParking">
  <h2 align="center">
  Parking id:<br>
  <input type="text" name="id" id="id" readonly="readonly" style="background-color: rgba(0,0,0,0.6); color: rgb(255,255,255);">
  <br>
  Parking name:<br>
  <input type="text" name="parkingName" id="parkingName" style="background-color: rgba(0,0,0,0.6); color: rgb(255,255,255);">
  <br>
  Parking address:<br>
  <input type="text" name="parkingAddress" id="parkingAddress" style="background-color: rgba(0,0,0,0.6); color: rgb(255,255,255);">
  <br>
  Number of places:<br>
  <input type="text" name="noOfPlaces" id="noOfPlaces" style="background-color: rgba(0,0,0,0.6); color: rgb(255,255,255);">
  <br>
  Free places:<br>
  <input type="text" name="freePlaces" id="freePlaces" style="background-color: rgba(0,0,0,0.6); color: rgb(255,255,255);">
  <br><br>
  <input type="submit" name="submit" value="Submit" style="background-color: rgba(0,0,0,0.6); color: rgb(255,255,255);">
  </h2>
</form>
